...
``func`` call, which replaces it so that ``start`` is never an output,
...
There's also a really old Python2-only package in PyPI called functional_\ ,
...
But, even for us who like to think on these concepts as ready to use
